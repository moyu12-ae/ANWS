...
**目标**: 调用 `design-reviewer` skill，从系统设计、运行模拟、工程实现三个维度审查设计文档。

> [!IMPORTANT]
> **设计审查逻辑已下沉到 `design-reviewer` skill**。
> challenge 工作流只负责编排和合并结果，不再内联审查逻辑。

### 执行

1. **调用 skill**: `design-reviewer`
   - 输入: `{TARGET_DIR}` 下的 `01_PRD.md`, `02_ARCHITECTURE_OVERVIEW.md`, `03_ADR/`, `04_SYSTEM_DESIGN/`
   - 输出: 设计审查发现清单（含严重度分级 + 文档段落关联）

2. **收集发现**: 将 `design-reviewer` 的输出暂存，待 Step 5 合并。

---

## Step 3.5: 任务审查 (Task Review — 条件触发)

**目标**: 如果 `05_TASKS.md` 存在，调用 `task-reviewer` skill 审查任务质量和完备性。

> [!IMPORTANT]
> **此步骤为条件触发**——只有项目已进入 blueprint 阶段（`05_TASKS.md` 存在）才执行。
> 否则跳过此步，直接进入 Step 4。

### 触发条件

```
IF {TARGET_DIR}/05_TASKS.md EXISTS
  → 执行 task-reviewer
ELSE
  → 跳过，记录 "Task review skipped: 05_TASKS.md not found"
```

### 执行

1. **调用 skill**: `task-reviewer`
   - 输入: `{TARGET_DIR}` 下的 `05_TASKS.md`, `01_PRD.md`, `02_ARCHITECTURE_OVERVIEW.md`, `03_ADR/`
   - 输出: 任务审查报告（6 大检测 Pass + REQ 覆盖率 + US 完整性 + 问题清单）

2. **收集发现**: 将 `task-reviewer` 的输出暂存，待 Step 5 合并。
...
1. **设计审查** (design-reviewer skill) — 系统设计 / 运行模拟 / 工程实现 三维度
2. **任务审查** (task-reviewer skill, 条件触发) — 重复 / 歧义 / 欠详述 / 不一致 / 覆盖率 / 质量粒度 六大 Pass
