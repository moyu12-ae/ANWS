...
└── anws/                          # npm 包根目录
    ...
    │   ├── update.js              # anws update — 更新托管文件、触发 diff 生成和 changelog 写入
    │   ├── diff.js                # [新增] 文件级 + 内容级 diff 生成，支持颜色 ANSI 输出
    │   ├── changelog.js           # [新增] 生成 anws/changelog/v{VERSION}.md 升级记录
    │   ├── copy.js                # 通用递归文件复制工具
...
`anws` 是一个**极简 CLI 工具**。`diff.js` 和 `changelog.js` 作为 `cli-system` 的内部模块存在，不构成独立系统——它们没有独立的部署单元和数据边界。拆分更多系统会导致**过度设计**。
