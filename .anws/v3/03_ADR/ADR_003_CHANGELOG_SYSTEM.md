...
### 决策 4: changelog 历史采用“同版本不可覆盖”策略

**选择**: `anws update` 与 `anws update --check` 在执行前都必须检测 `anws/changelog/` 中的最新已记录版本；如果该版本已经等于当前 CLI 版本，则直接返回 "Already up to date."，不再覆盖既有 `v{VERSION}.md`。

**选择理由**:
- changelog 是 `/upgrade` 判断升级类型与升级影响范围的历史依据，覆盖会破坏可追溯性
- 同版本重复执行通常意味着“已更新完成”而非“需要重建历史”
- 让 `--check` 与正式更新在版本判断上保持一致，可避免预览与真实执行产生认知偏差

### 决策 5: `AGENTS.md` 采用“稳定提示词区 + AUTO 运行态区块”分区模型

**选择**: 将根目录 `AGENTS.md` 建模为单文件双区域资源：
- 区块外内容是模板稳定区，用于存放长期有效的提示词、协议和工作流注册表
- `AUTO:BEGIN` ~ `AUTO:END` 之间是运行态区块，用于存放项目当前状态、结构、导航与任务上下文

**选择理由**:
- 稳定提示词与项目运行态变化频率不同，分区后可以分别演进
- 工作流可以安全更新运行态区块，而不破坏用户维护的稳定协议文本
- 后续 `anws update` 可以在单文件内执行分区合并，而非粗暴覆盖整个 `AGENTS.md`
...
- `src/anws/templates/AGENTS.md` — 引入稳定提示词区与 `AUTO` 运行态区块边界
...
3. **幂等性** — 相同版本多次运行 `anws update` 或 `anws update --check` 时，直接返回 "Already up to date."，不覆盖已存在的同名 changelog 文件
4. **目录自动创建** — `anws/changelog/` 不存在时自动创建，不要求用户手动操作
5. **单文件分区保护** — 对 `AGENTS.md` 的后续更新必须尊重 `AUTO:BEGIN` / `AUTO:END` 边界，不允许破坏运行态区块
...
- `src/anws/templates/AGENTS.md` — `AUTO` 区块模板结构
