# 系统架构总览 (Architecture Overview)

**项目**: `anws` — Anws CLI
**版本**: 3.0
**日期**: 2026-03-14
**关联 ADR**: `03_ADR/ADR_001_TECH_STACK.md`, `03_ADR/ADR_003_CHANGELOG_SYSTEM.md`

---

## 1. 系统上下文 (System Context)

### 1.1 C4 Level 1 — 系统上下文图

```mermaid
graph TD
    Dev[👤 开发者\n用户] -->|npm install -g anws| NPM[📦 npm Registry\n包托管]
    Dev -->|anws init / anws update| CLI[🛠️ anws CLI\n本系统]
    CLI -->|写入文件| TargetProject[📁 目标项目\n.agents/ 目录 + AGENTS.md + anws/changelog/]
    Maintainer[🔧 维护者] -->|npm publish| NPM
    
    style CLI fill:#ffe4b5,stroke:#d4a017
    style NPM fill:#e1f5ff,stroke:#0078d4
    style TargetProject fill:#e1ffe1,stroke:#228b22
```

### 1.2 关键角色 (Key Actors)

| 角色 | 描述 | 交互方式 |
|------|------|---------|
| **开发者 (用户)** | 想在项目中使用工作流系统的人 | 运行 `npm install -g anws`，然后 `anws init` |
| **维护者** | Antigravity 系统的开发和发布者 | 更新 templates，运行 `npm publish` |

### 1.3 外部系统 (External Systems)

| 系统 | 类型 | 交互 |
|------|------|------|
| npm Registry | 外部服务 | 维护者发布包；用户下载安装 |
| 目标项目文件系统 | 用户本地 | CLI 写入 `.agent/` 目录 |
| GitHub Releases | 外部服务（备选分发）| 维护者上传 `.zip`；用户手动下载 |

---

## 2. 系统清单 (System Inventory)

### System 1: CLI System
**系统 ID**: `cli-system`

**职责 (Responsibility)**:
- 解析命令行参数（`init` / `update` / `--version` / `--help`）
- 路由到对应命令处理器
- 处理用户交互（Y/N 确认）
- 输出格式化的终端反馈（包含颜色 diff 输出）
- 在 `update` 流程中执行版本检测，决定是生成 changelog 还是直接返回 "Already up to date."
- 在后续 `AGENTS.md` 更新策略中执行稳定区与 `AUTO` 运行态区块的边界保护

**边界 (Boundary)**:
- **输入**: `process.argv` (命令行参数, 包含 `--check` flag), 用户的 stdin 输入 (readline)
- **输出**: stdout 打印（文件列表、确认提示、错误信息、diff 预览）
- **依赖**: `template-bundle-system`（读取内嵌模板），目标项目文件系统（写入）

**关联需求**: [REQ-001] [REQ-002] [REQ-003] [REQ-004] [REQ-006]

**技术栈**:
- Runtime: Node.js ≥ 18
- 参数解析: `node:util` parseArgs
- 交互: `node:readline`
- 文件操作: `node:fs/promises`, `node:path`
- 模块格式: CommonJS

**源码根目录**: `src/anws/bin/` + `src/anws/lib/`

**设计文档**: `04_SYSTEM_DESIGN/cli-system.md` (待创建)

---

### System 2: Template Bundle
**系统 ID**: `template-bundle`

**职责 (Responsibility)**:
- 存储 Antigravity Workflow System 的完整 `.agent/` 目录内容
- 内嵌于 npm 包，随包版本一起发布和更新
- 维护 **managed files 清单**（决定哪些文件属于我们管理）
- 提供模板内容供 diff 生成比对
- 提供根目录 `AGENTS.md` 模板，其中稳定提示词区与 `AUTO` 运行态区块拥有明确边界

**边界 (Boundary)**:
- **输入**: 维护者更新 `templates/.agents/` 内容 + `npm publish`
- **输出**: `cli-system` 读取后复制到目标项目
- **依赖**: 无（静态文件）

**关联需求**: [REQ-002] [REQ-003] [REQ-004] [REQ-006] [REQ-007]

**技术栈**: 纯静态文件，无运行时技术

**源码根目录**: `src/anws/templates/`

**Managed Files 清单机制**:
清单维护在 `src/anws/lib/manifest.js` 中，列出所有属于本包的文件相对路径。更新时仅覆盖清单内的文件，清单外的用户文件不受影响。对于根目录 `AGENTS.md`，v3 将其建模为“单文件、双区域”资源：区块外为模板稳定区，`AUTO:BEGIN` ~ `AUTO:END` 之间为运行态区块，后续更新必须尊重这一分区边界。

**设计文档**: 无需单独设计文档（纯文件复制）

---

## 3. 系统边界矩阵 (System Boundary Matrix)

| 系统 | 输入 | 输出 | 依赖系统 | 被依赖 | 关联需求 |
|------|------|------|---------|--------|---------|
| CLI System | `argv`, `stdin` | `stdout`, 文件写入, changelog 生成 | Template Bundle | — | REQ-001~004, REQ-006, REQ-007 |
| Template Bundle | 维护者更新文件 | 文件内容（供 CLI 读取/diff） | — | CLI System | REQ-002~004, REQ-006, REQ-007 |

---

## 4. 系统依赖图 (System Dependency Graph)

```mermaid
graph LR
    CLI[CLI System\ncli.js + lib/] -->|"path.resolve(__dirname, '../templates')"| Templates[Template Bundle\ntemplates/.agent/]
    CLI -->|"fs.cp / fs.copyFile"| TargetFS[目标项目 .agent/]
    Templates -.->|"随 npm 包版本更新"| NPM[npm Registry]
    
    style CLI fill:#ffe4b5
    style Templates fill:#e1f5ff
    style TargetFS fill:#e1ffe1
    style NPM fill:#f5f5f5
```

---

## 5. 物理代码结构 (Physical Code Structure)

```
src/
└── anws/                          # npm 包根目录
    ├── package.json               # name: "anws", bin: { anws: "./bin/cli.js" }
    ├── README.md                  # 用户文档（安装、使用、GitHub 备选）
    ├── bin/
    │   └── cli.js                 # #!/usr/bin/env node — 主入口，parseArgs (--check flag)，命令路由
    ├── lib/
    │   ├── init.js                # anws init — 检测冲突、写入文件、打印摘要
    │   ├── update.js              # anws update — 版本检测、更新托管文件、协调 AGENTS 边界与 changelog 写入
    │   ├── diff.js                # 文件级 + 内容级 diff 生成，支持逐行前后对比与颜色 ANSI 输出
    │   ├── changelog.js           # 生成 anws/changelog/v{VERSION}.md，并检测最新已记录版本
    │   ├── copy.js                # 通用递归文件复制工具
    │   └── manifest.js            # MANAGED_FILES 常量数组（托管文件清单）
    └── templates/
        ├── .agents/               # 内嵌的完整工作流模板
        │   ├── workflows/
        │   │   ├── genesis.md
        │   │   ├── blueprint.md
        │   │   ├── forge.md
        │   │   └── ...
        │   ├── skills/
        │   │   └── ...
        │   └── AGENTS.md
        └── AGENTS.md              # 根目录 AGENTS.md 模板

# 用户项目中由 anws 管理的目录结构 (示例)
anws/
├── v1/                            # 架构文档 v1
├── v2/                            # 架构文档 v2 (过 /genesis 升级)
└── changelog/                     # 升级记录 (请勿删除)
    ├── README.md              # 保护提示文件
    └── v1.4.0.md              # anws update 自动生成

genesis/v2/                        # 本架构文档（只读参考）
```

---

## 6. 核心执行流程 (Key Execution Flows)

### Flow A: `anws init`（无 `.agent/`）

```mermaid
sequenceDiagram
    participant User
    participant CLI
    participant FS as 文件系统

    User->>CLI: anws init
    CLI->>FS: 检查 .agent/ 是否存在
    FS-->>CLI: 不存在
    CLI->>FS: 递归复制 templates/.agent/ → ./.agent/
    FS-->>CLI: 写入成功
    CLI->>User: ✔ Done! N files written
```

### Flow B: `anws init`（`.agent/` 已存在）

```mermaid
sequenceDiagram
    participant User
    participant CLI
    participant FS as 文件系统

    User->>CLI: anws init
    CLI->>FS: 检查 .agent/ 是否存在
    FS-->>CLI: 存在
    CLI->>FS: 比对 managed files 清单
    FS-->>CLI: 找到 N 个托管文件
    CLI->>User: ⚠ Overwrite? [y/N]
    User->>CLI: y (或 N)
    alt 用户确认 y
        CLI->>FS: 仅覆盖 managed files
        CLI->>User: ✔ Updated N files
    else 用户取消 N
        CLI->>User: Aborted. No files changed.
    end
```

### Flow C: `anws update`

```mermaid
sequenceDiagram
    participant User
    participant CLI
    participant Diff as diff.js
    participant Changelog as changelog.js
    participant FS as 文件系统

    User->>CLI: anws update
    CLI->>FS: 检查 .agents/ 是否存在
    CLI->>Changelog: detectUpgrade(cwd, version)
    Changelog-->>CLI: latestVersion / needUpgrade
    alt 已记录当前版本
        CLI->>User: Already up to date.
    else 需要升级
        CLI->>User: ☠ Overwrite? [y/N]
        User->>CLI: y
        CLI->>Diff: 对比模板 vs 当前文件
        Diff-->>CLI: 变更清单 (added/modified/deleted)
        CLI->>FS: 覆盖 managed files
        CLI->>Changelog: generateChangelog(changes, version)
        Changelog->>FS: 写入 anws/changelog/v{VERSION}.md
        CLI->>User: ✔ Updated N files + Generated changelog
    end
```

### Flow D: `anws update --check`

```mermaid
sequenceDiagram
    participant User
    participant CLI
    participant Diff as diff.js
    participant FS as 文件系统

    User->>CLI: anws update --check
    CLI->>Changelog: detectUpgrade(cwd, version)
    Changelog-->>CLI: latestVersion / needUpgrade
    alt 已记录当前版本
        CLI->>User: Already up to date.
    else 需要升级
        CLI->>FS: 读取模板文件
        CLI->>FS: 读取当前 .agents/ 文件
        CLI->>Diff: 对比全量 managed files
        Diff-->>CLI: 内容级 diff (新增/修改/删除)
        CLI->>User: 输出颜色 diff 预览（不写任何文件）
    end
```

---

## 7. 拆分原则与理由 (Decomposition Rationale)

### 为什么只有 2 个系统？

`anws` 是一个**极简 CLI 工具**。`diff.js` 和 `changelog.js` 作为 `cli-system` 的内部模块存在，不构成独立系统——它们没有独立的部署单元和数据边界。`AGENTS.md` 的分区更新也只是 CLI System 在单文件内执行的边界策略，而不是新的系统。拆分更多系统会导致**过度设计**。

系统拆分标准（对照本项目）：

| 标准 | 本项目评估 |
|------|-----------|
| 技术栈差异 | CLI(Node.js代码) ≠ Template(静态文件) → 2个系统 ✓ |
| 独立部署 | CLI 和 Template 捆绑在同一 npm 包 → 无需额外拆分 |
| 职责差异 | CLI=执行逻辑，Template=数据 → 天然分离 |
| 变化频率 | Template 更新频繁，CLI 逻辑相对稳定 → 分离有价值 |

### 为什么没有"数据库系统"?

无状态 CLI 工具，所有"数据"就是文件，无需数据库。

### 为什么没有"后端 API 系统"?

纯本地工具，无网络请求（设计约束之一）。

---

## 8. 技术栈总览 (Technology Stack Overview)

| Layer | Technology | 用于 |
|-------|-----------|------|
| **CLI Runtime** | Node.js ≥ 18 | CLI System |
| **参数解析** | `node:util` parseArgs | CLI System |
| **交互 Prompt** | `node:readline` | CLI System |
| **文件操作** | `node:fs/promises`, `node:path` | CLI System |
| **模块格式** | CommonJS | CLI System |
| **包分发** | npm Registry | 两个系统 |
| **模板内容** | Markdown/JSON 静态文件 | Template Bundle |
| **Diff 生成** | Node.js 内置 (`fs/promises`) 逐行对比 | CLI System |
| **ANSI 颜色** | 原生 ANSI 转义码（无第三方库） | CLI System |
